...
function! s:CreateMenu(sub, expanded)
  ...
  exec 'amenu <silent> ' . a:sub . '&Perforce' . fileGroup . '&VDiff :PVDiff<CR>'
  exec 'amenu <silent> ' . a:sub . '&Perforce' . fileGroup . 'VDiff&2 :PVDiff2<CR>'
  exec 'amenu <silent> ' . a:sub . '&Perforce' . fileGroup .
  ...
  if a:expanded
    ...
    let p4Presets = split(perforce#PFGet('g:p4Presets'), ',')
    if len(p4Presets) > 0
      let index = 0
  ...
  exec 'amenu <silent> ' . a:sub . '&Perforce.&Help :PluginHelp<CR>'
endfunction
endif
" }}}
