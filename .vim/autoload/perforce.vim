...
let s:settings = split('ClientRoot,CmdPath,Presets,' .
      ...
      \ 'BufHidden,Depot,Autoread,UseClientViewMap,DefaultPreset,DebugMessages,UseP4Cmds', ',')
...
function! s:VDiffHandler()
  ...
  if firstFile == ''
    if s:_('UseP4Cmds')
      let firstFile = s:ConvertToDepotPath_shreedr(secondFile)
    else
...
function! s:ConvertToDepotPath_shreedr(filepath)
  call s:EchoDebugMessage(" filepath = ".a:filepath)
  if s:IsDepotPath(a:filepath)
    call s:EchoDebugMessage("already depot path")
    let file = a:filepath
  else
    let file = genutils#CleanupFileName(a:filepath)
  let path = genutils#Escape(file, ' &|#')
  call s:EchoDebugMessage(" path = ".path)
  "return path
  let cmd1 = "setenv temp1 `p4 fstat ".path." | grep depotFile | sed -e 's;\.\.\. depotFile ;;g'`; echo -n $temp1"
  let cmd2 = "setenv temp2 `p4 fstat ".path." | grep headRev | sed -e 's;\.\.\. headRev ;;g'`; echo -n $temp2"
  call s:EchoDebugMessage("cmd1 = ".cmd1)
  call s:EchoDebugMessage("cmd2 = ".cmd2)
  let fileName = system(cmd1)
  let fileRev = system(cmd2)
  call s:EchoDebugMessage(" fileRev = ".fileRev)
  call s:EchoDebugMessage(" fileName = ".fileName)
  let fileName = fileName."#".fileRev
  call s:EchoDebugMessage(" return fileName = ".fileName)
  return fileName
endfunction
...
function! s:VDiff2Handler()
  ...
  let s:p4Arguments = s:GetDiff2Args()
  if s:_('UseP4Cmds')
    let firstFile = s:ConvertToDepotPath_shreedr(s:p4Arguments[0])
    let secondFile = s:ConvertToDepotPath_shreedr(s:p4Arguments[1])
  else
    let firstFile = s:ConvertToDepotPath(s:p4Arguments[0])
    let secondFile = s:ConvertToDepotPath(s:p4Arguments[1])
  endif
  call s:VDiffImpl(firstFile, secondFile, 0)
endfunction
...
function! s:VDiffImpl(firstFile, secondFile, preferDepotPaths)
  ...
  call s:EchoDebugMessage("VDiffImpl::firstFile = ".firstFile)
  call s:EchoDebugMessage("VDiffImpl::secondFile = ".secondFile)
...
function! s:EchoDebugMessage(msg)
  if s:_('DebugMessages')
    redraw | echom "p4DEBUG" a:msg
  endif
endfunction
...
function! s:ConvertToDepotPath(path)
  ...
  if s:IsDepotPath(a:path)
    ...
    if s:IsFileUnderDepot(fileName)
      call s:EchoDebugMessage("ConvertToDepotPath::fileName= ".fileName)
      if s:_('UseClientViewMap')
        call s:EchoDebugMessage("ConvertToDepotPath0.1:fileName= ".fileName)
        call s:CondUpdateViewMappings()
        for nextMap in s:toDepotMapping[s:_('p4Client')]
          let [lhs, rhs] = nextMap
          call s:EchoDebugMessage("ConvertToDepotPath::[lhs,rhs] = ".lhs.",".rhs)
          if fileName =~# '\V'.lhs
            let fileName = substitute(fileName, '\V'.lhs, rhs, '')
            break
          endif
      ...
      call s:EchoDebugMessage("ConvertToDepotPath1::fileName= ".fileName)
      if ! s:IsDepotPath(fileName)
        call s:EchoDebugMessage("ConvertToDepotPath2::fileName= ".fileName)
        "let fileName = substitute(fileName, '^'.s:_('ClientRoot'),
        "      \ '//'.s:_('Depot'), '')
        let fileName = substitute(fileName, s:_('ClientRoot').'\/', '', '')
        call s:EchoDebugMessage("ConvertToDepotPath2.5::fileName= ".fileName)
        let fileName = '//'.s:_('Depot').'/'.fileName
        call s:EchoDebugMessage("ConvertToDepotPath3::fileName= ".fileName)
      endif
    endif
  endif
  return fileName
